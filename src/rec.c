 ...
 */
void clear2color(ALLEGRO_BITMAP *bmp, int color) {
	rectfill(bmp, 0, 0, al_get_bitmap_width(bmp) - 1, al_get_bitmap_height(bmp) - 1, color);
}
...
int GetRes(char *aimsn) {
	...
	{ d_box_proc, 0, 0, 288, 176, 0, 7, 0, 0, 0, 0, NULL },
						...
						{ d_list_proc, 96, 27, 80, 70, 0, 7, 'r', D_EXIT, 0, 0, GetRes_List }, {
								DY_check_proc, 24, 108, 144, 8, 0, 7, 'm', D_SELECTED, 0, 0,
								"Play &MIDI" },
...
int GetOpts(char *aimsn, ALLEGRO_BITMAP *todlogo) {
	DIALOG dlg[] = {
	/* (dialog proc)     (x)   (y)   (w)   (h)   (fg)  (bg)  (key) (flags)  (d1)  (d2)  (dp) */
	{ d_box_proc, 0, 0, 320, 200, 0, 0, 0, 0, 0, 0, NULL },
						...
						{ d_button_proc, 170, 184, 60, 12, 63, 0, 27, D_EXIT, 0, 0, "Cancel" }, {
								DY_check_proc, 24, 120, 144, 8, 63, 0, 't', 0, 0, 0,
								"&The New Tetanus" },
						{ DY_check_proc, 24, 132, 144, 8, 63, 0, '2', 0, 0, 0,
							"&2-player (prealpha)" },
...
void SetupLight1(void) {
	...
	for (y = 0; y < 256; y++) {
		...
		for (x = 0; x < 256; x++) {
			if (line[x] == c1) {
				al_fixed f = fsqrt((x - 128) * (x - 128) + (y - 128) * (y - 128)); /* between 0 and 2 */
				al_fixed cosine = hgrcos(f << 10);
				unsigned c = (cosine + 0x10000 + rand() % 0x1000) * 15 / 32768;
				if (c > 63)
					c = 63;
				line[x] = c + 64;
			} else if (line[x] == c2) {
				al_fixed f = fsqrt((x - 128) * (x - 128) + (y - 128) * (y - 128))
						+ (fcos(x << 20) + fcos(y << 20)) / 64 + 0x10000;
				/* between 0 and 4 */
				al_fixed cosine = hgrcos(f << 11);
				unsigned c = (cosine + 0x10000 + rand() % 0x1000) * 15 / 32768;
				if (c > 63)
					c = 63;
				line[x] = c + 128;
...
void ThetaLight(al_fixed theta)
{
	unsigned r, g, b;
...
void InitSidesPhi(Seven *seven, al_fixed phi, al_fixed zBase) {
	int i;
	int wHt = al_get_bitmap_height(seven->frontBuf);
	al_fixed m, zCos;
	...
	for (i = 0; i < wHt; i++) {
		al_fixed scanline = fdiv(i - wHt / 2, wHt / 2);
		al_fixed mline = fmul(m, scanline);
		al_fixed zLine = fmul(zCos, scanline);
		al_fixed y1 = fdiv(zLine, mline + 0x10000);
		al_fixed x1 = fdiv(zBase, mline + 0x10000);
...
void SevenRender(Seven *seven) {
	...
	int x = 0, y = 0;
	al_fixed xgap = 0, ygap = 0, sx, sy;
	int width = seven->rightSide - seven->leftSide;
	int height = al_get_bitmap_height(seven->frontBuf);
	unsigned char *dst;
...
void RotateSides(Seven *seven, int theta) {
	al_fixed lx, ly, rx, ry;
	al_fixed c = hgrcos(theta);
	al_fixed s = hgrsin(theta);
	int i;
	int wHt = al_get_bitmap_height(seven->frontBuf);
...
void DisplaceSides(Seven *seven, al_fixed x, al_fixed y) {
	int i;
	int wHt = al_get_bitmap_height(seven->frontBuf);
...
void DisplaceScanlines(Seven *seven, int ticks, int a, int b, int c, int d, al_fixed dtheta,
		al_fixed amplitude, al_fixed noise, al_fixed heat) {
	al_fixed lx, ly, rx, ry;
	al_fixed xOff, yOff;
	int i, y;

	int wHt = al_get_bitmap_height(seven->frontBuf);
	al_fixed dheat = 0x8000000 / wHt;
	al_fixed theta = (ticks << 14) & 0x00ffffff;
	al_fixed heatTheta = (ticks << 16) & 0x00ffffff;

	dtheta /= wHt;

	for (i = 0; i < wHt; i++) {
		y = i - wHt / 2;
		xOff = ((a * y / wHt + b) * y / wHt + c) * y / wHt + d + fmul(hgrcos(theta), amplitude);
...
void textout_shadow(ALLEGRO_BITMAP *screen, const ALLEGRO_FONT *font, const char *str, int x, int y,
		int c1, int c2) {
	al_draw_text((ALLEGRO_FONT *) font, c2, x + 1, y + 1, ALLEGRO_ALIGN_LEFT, (char *) str);
	al_draw_text((ALLEGRO_FONT *) font, c1, x, y, ALLEGRO_ALIGN_LEFT, (char *) str);
}

static void wrapblit(ALLEGRO_BITMAP *a, ALLEGRO_BITMAP *b, int c, int d, int e, int f, int g, int h) {
...
static void PlayGame(Seven *seven, ScreenPos *pos, const char *aimsn) {
	...
	const int lead = al_get_display_height() / 14;
	...
	while (!done) {
		...
		for (i = 0; i < nPlayers; i++) {
			...
			DisplaceScanlines(&(seven[i]), lastGameloop, pos[i].a, pos[i].b, pos[i].c, pos[i].d,
					pos[i].sinusFreq, pos[i].amp >> 8, pos[i].noise, pos[i].heat);
		...
		text_mode(-1);
		textprintf(seven->frontBuf, lucid, 0, lead * 13, 0, "%4ufps %4lubeat %3udose %3umana",
...
int main(void) {
	...
	while (Login(aimsn) >= 0) {
		while (GetOpts(aimsn, GetResource(dat, "LOGIN_BMP")) >= 0) {
			...
			if (nPlayers == 2) {
				seven[0].nextX = al_get_display_width() * 2 / 5;
				seven[1].nextX = al_get_display_width() / 2;
				seven[0].leftSide = 0;
				seven[0].rightSide = seven[1].leftSide = al_get_display_width() / 2;
				seven[1].rightSide = al_get_display_width();
			} else {
				seven[0].nextX = al_get_display_width() * 1 / 5;
				seven[0].leftSide = 0;
				seven[0].rightSide = al_get_display_width();
			}
	...
	al_destroy_bitmap(seven[0].frontBuf);
